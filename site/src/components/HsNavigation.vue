  ...
  <header class="header-black">
    <div class="header-wrapper">
      ...
      <div class="projectNavigation">
        ...
        <div @click="hideProjectView" class="link breadCrumb">
          {{ global.breadCrumb }}
        </div>
      </div>
    </div>
    <div class="">
      <Transition name="fade" @after-leave="onAfterLeave">
        <nav
          class="navigation"
          :class="{ active: global.menuIsActive }"
          v-if="global.menuIsActive"
        >
          <ul class="navigation-list">
            <li :key="1" class="navigation-list-item" style="transition-delay: 0.2s">
              <RouterLink
                @click="toggleMenue($event, 'me')"
                :to="{ path: '/page', query: { id: 'me' } }"
                class="navigation-link"
                >stykki</RouterLink
              >
            </li>

            <li :key="2" class="navigation-list-item" style="transition-delay: 0.2s">
              <RouterLink
                @click="toggleMenue($event, 'prozess')"
                :to="{ path: '/page', query: { id: 'prozess' } }"
                class="navigation-link"
                >herstellungsprozess</RouterLink
              >
            </li>

            <li :key="3" class="navigation-list-item" style="transition-delay: 0.2s">
              <RouterLink
                @click="toggleMenue($event, 'Lochkamerafotografien')"
                :to="{ path: '/page', query: { id: 'Lochkamerafotografien' } }"
                class="navigation-link"
                >lochkamerafotografien</RouterLink
              >
            </li>

            <li class="navigation-list-item" style="transition-delay: 0.2s">
              <RouterLink
                @click="toggleMenue($event, 'auszug')"
                to="/auszug"
                class="navigation-link"
                >auszug
              </RouterLink>
